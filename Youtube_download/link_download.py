...
def install_yt_dlp():
    """Устанавливает yt-dlp через pip"""
    print("Установка yt-dlp...")
    subprocess.check_call([sys.executable, "-m", "pip", "install", "yt-dlp"])

def check_available_formats(url, target_height=1080):
    """
    Проверяет доступные форматы для видео и возвращает лучшее доступное качество
    
    Args:
        url (str): ссылка на YouTube видео
        target_height (int): желаемое качество (1080, 1440, 2160)
    
    Returns:
        dict: информация о лучшем доступном качестве
    """
    try:
        import yt_dlp
...
def download_guaranteed_quality(url, output_path=".", target_quality="1080"):
    ...
    print(f"\n Анализ доступных форматов для {url}")
    ...
    if formats_info['best_h264'] >= target_height:
        ...
        #2: Скачиваем в лучшем качестве (VP9/AV1) и конвертируем в H.264
...
def download_multiple_videos_guaranteed(urls, output_path=".", target_quality="1080"):
    ...
    print(f"Папка сохранения: {output_path}")
    ...
    for i, url in enumerate(urls, 1):
        ...
        print(f"\nПрогресс: {i}/{len(urls)}, успешно: {len(statistics['success'])}, ошибок: {len(statistics['failed'])}")

    print(f"Скачано: {len(statistics['success'])}")
    print(f"Не удалось скачать: {len(statistics['failed'])}")
    
    if statistics['failed']:
        print("\nСписок видео, которые не удалось скачать в нужном качестве:")
        for i, url in enumerate(statistics['failed'], 1):
            print(f"  {i}. {url}")
    
    print(f"\nВсе видео сохранены в: {os.path.abspath(output_path)}")
