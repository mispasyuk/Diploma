...
class EyeRegionExtractor:
    ...
    def _crop_eye_region(self, frame, eye_points):
        """
        Обрезка области вокруг глаза
        
        Args:
            frame: кадр
            eye_points: координаты landmarks глаза
            
        Returns:
            обрезанная область глаза
        """
        ...
        # Resize to 128x128 (как в статье)
...
def extract_frames_features(video_folder_path, extractor, output_dir, max_frames=64):
    """
    Извлечение признаков из папки с кадрами
    
    Args:
        video_folder_path: путь к папке с кадрами видео
        extractor: экземпляр EyeRegionExtractor
    ...
    # Получаем все JPG файлы и сортируем их
    ...
    for frame_idx, frame_path in enumerate(tqdm(frame_files, desc=f"Processing {video_name}", leave=False)):
        ...
        # Извлекаем области глаз (лицо есть на всех кадрах)
        eye_data = extractor.extract_eye_regions(frame)
        
        # Сохраняем области глаз
        ...
        # Сохраняем информацию о landmarks
        frame_info = {
...
def process_dataset(frames_root_dir, output_dir, predictor_path):
    ...
    Args:
        frames_root_dir: корневая директория с папками кадров (fakeFramesFacesV2)
        output_dir: директория для сохранения признаков
